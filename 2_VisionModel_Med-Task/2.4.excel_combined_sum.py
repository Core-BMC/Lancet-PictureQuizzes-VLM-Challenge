...
class ExcelProcessor:
    def __init__(self, base_models, variations, max_num=227):
        self.base_models = base_models
        self.variations = variations
        self.temperatures = [0]
        self.tries = range(1, 2)
    ...
    def extract_info_from_text(text):
        try:
            ...
            return {
                'TypeOfMedicalImaging': data.get("1_TypeOfMedicalImaging", ""),
                'SpecificImagingSequence': data.get("2_SpecificImagingSequence", ""),
                'UseOfContrast': data.get("3_UseOfContrast", ""),
                'ImagePlane': data.get("4_ImagePlane", ""),
                'PartOfTheBodyImaged': data.get("5_PartOfTheBodyImaged", ""),
                'LocationOfAbnormalFinding': data.get("6_LocationOfAbnormalFinding", "")
            }
    ...
    def process_folders(self, combined_excel_file_path):
        ...
        for folder_path in self.folder_paths:
            ...
            ws = wb.active
            ws.append([
                'case_number', 'TypeOfMedicalImaging', 'SpecificImagingSequence', 
                'UseOfContrast', 'ImagePlane', 'PartOfTheBodyImaged', 'LocationOfAbnormalFinding'
            ])
            ...
            for i in range(1, self.max_num + 1):
                ...
                try:
                    with open(file_path, 'r', encoding='utf-8') as file:
                        ...
                        if extracted_data:
                            ws.append([
                                i, extracted_data['TypeOfMedicalImaging'],
                                extracted_data['SpecificImagingSequence'], extracted_data['UseOfContrast'],
                                extracted_data['ImagePlane'], extracted_data['PartOfTheBodyImaged'],
                                extracted_data['LocationOfAbnormalFinding']
                            ])
                        else:
                            ws.append([i, '', '', '', '', '', ''])
...
def main():
    ...
    original_processor = ExcelProcessor(
        base_models=['gpt4v', 'gpt4o', 'gemini',
                     'gemini_flash', 'Claude', 'Claude_35'],
        variations=['result']
    )
    original_processor.process_folders('original-decribe_combined_sum.xlsx')
